...
class Network(Base):
    ...
    def __repr__(self):
        """
        Returns a string representation of the Network instance.

        Example:
            <Network(id=1, username='johndoe', email='john@example.com')>
        """
        return f"<Network(id={self.id}, username='{self.username}', email='{self.email}')>"


class NetworkCreate(BaseModel):
    """
    Pydantic schema for creating a new Network.
